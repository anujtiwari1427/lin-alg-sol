...
class TestCalculationAPIs(unittest.TestCase):
    ...
    def test_new_matrix_operations(self):
        # LU Decomposition
        r = self.client.post('/api/matrix/calculate', json={
            'operation': 'lu',
            'matrix_a': [[4, 3], [6, 3]]
    ...
    def test_new_vector_operations(self):
        # Orthogonality
        r = self.client.post('/api/vector/calculate', json={
        ...
        data = json.loads(r.data)
        self.assertTrue(data['success'])
        self.assertTrue(data['result']['is_orthogonal'])
        ...
        data = json.loads(r.data)
        self.assertTrue(data['success'])
        self.assertEqual(data['result'], [5.0, 8.0])

    def test_new_linear_equations_solvers(self):
        # Gauss-Jordan
        r = self.client.post('/api/linear-equations/calculate', json={
            'method': 'gauss_jordan',
            'coefficients': [[2, 1], [1, -1]],
